...
// If your port is set to 80,如果端口设置为80，
// use administrator privileges to execute the command line.使用管理员权限执行命令行。
// For example, Mac: sudo npm run例如，Mac:sudo npm run
// You can change the port by the following methods:您可以通过以下方法更改端口
// port = 9528 npm run dev OR npm run dev --port = 9528端口=9528 npm运行开发或npm运行开发--端口=9528
...
module.exports = {
  ...
  chainWebpack(config) {
    ...
    // set preserveWhitespacevue webpack配置手动添加 项目打包时候去掉空格减少项目体积
    config.module
      .rule('vue')
      .use('vue-loader')
      .loader('vue-loader')
